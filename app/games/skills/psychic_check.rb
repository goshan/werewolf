class PsychicCheck < Skill

  def prepare
    res = SkillResponsePanel.new 'psychic_check'
    res.select = SkillResponsePanel::SELECT_SINGLE
    res.button_push 'psychic_check'
    res.to_msg
  end
  ...
  def use(target)
    ...
    res = SkillResponseDialog.new 'psychic_checked'
    res.add_param 'target', target
    res.add_param 'side', player.role.side == :wolf ? :evil : :virtuous
    res.add_param 'role', player.role.name
    res.cannot_retry!
    res.to_msg
  end
